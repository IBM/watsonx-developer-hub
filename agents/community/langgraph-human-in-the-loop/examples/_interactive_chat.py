                ...
                for action, stage in user_loop:  # unsupported command support!
                    if action == "h" or action == "help":
                        ...
                        if self.thread_id is not None:
                            request_payload_json["thread_id"] = self.thread_id
                        ...
                        if self.stream:
                            for r in resp:
                                ...
                                if (
                                    ...
                                    and (thread_id := r.get("thread_id")) is not None
                                ):
                                    self.thread_id = thread_id

                                if (
                                    self.thread_id is not None
                                    and (thread_id := r.get("thread_id")) is None
                                ):
                                    self.thread_id = None
                                for c in r["choices"]:
                            ...
                            if (
                                thread_id := resp.get("body", resp).get("thread_id")
                            ) is not None:
                                self.thread_id = thread_id
                            else:
                                self.thread_id = None
