...
def create_config(template_file: str, target_file: str):
    ...
    if os.path.exists(template_file):
        ...
        raise FileNotFoundError(
            f"Configuration template file {template_file} does not exist!"
        )
    ...
    if deployment_section_name not in config.keys():
        raise KeyError(
            f"Configuration section {deployment_section_name} not found in {target_file}"
        )
    ...
    config[deployment_section_name]["online"]["parameters"]["url"] = config[
        deployment_section_name
    ]["watsonx_url"]
