    ...
    async def generate_async(context) -> dict:
        ...
        response = await workflow.run(
            inputs=[
                ...
                AgentWorkflowInput(
                    prompt=f"Search for a set of activities close to {input_prompt} from {today} that are appropriate in light of the weather conditions.",  # noqa: E501
                    expected_output="A list of activities including location and description that are weather appropriate.",
                ),
                AgentWorkflowInput(
                    prompt=f"Consider the weather report and recommended activities for the trip to {input_prompt} from {today} and provide a coherent summary.",  # noqa: E501
